...
(function() {
    ...
    function loadSidebarState() {
        const saved = window.sessionStorage.getItem('promptEditorSidebarState');
        if (saved) {
            try {
    ...
    function saveSidebarState() {
        window.sessionStorage.setItem('promptEditorSidebarState', JSON.stringify(sidebarState));
    }
    ...
    function loadDirectoryPrompts() {
        ...
        if (!directoryPromptsDiv || !window.currentPromptData || !window.currentPromptData.directory_info) {
            return;
        }
        
        const currentDirectory = window.currentPromptData.directory_info.path;
        ...
        fetch(`/api/prompts/directories/${encodeURIComponent(currentDirectory)}/prompts`)
            ...
            .then(prompts => {
                ...
                const otherPrompts = prompts
                    .filter(prompt => prompt.id !== window.promptId)
                    .sort((a, b) => {
    ...
    function initializeDragAndDrop() {
        ...
        promptItems.forEach(item => {
            item.addEventListener('dragstart', (e) => {
                if (!window.rawContentPane.classList.contains('show')) {
                    e.preventDefault();
                    if (window.showToast) {
        ...
        editorElement.addEventListener('drop', (e) => {
            ...
            if (!window.rawContentPane.classList.contains('show')) {
                if (window.showToast) {
                    window.showToast('Can only drop prompts in Raw Content view.', 'warning');
                }
                return;
            }
            ...
            const coords = window.editor.coordsChar({
                left: e.clientX,
                top: e.clientY
            });
            ...
            window.editor.replaceRange(inclusionText, coords);
            ...
            if (window.showToast) {
                window.showToast(`Inserted [[${promptName}]]`, 'success');
            }
        });
    }

    // Export functions to global scope for use in main script
